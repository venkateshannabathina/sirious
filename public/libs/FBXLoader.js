						...
						if ( curveNodesMap.has( child.ID ) ) {
							...
							if ( curveNode.curves.x !== undefined || curveNode.curves.y !== undefined || curveNode.curves.z !== undefined ) {
								...
								if ( layerCurveNodes[ i ] === undefined ) {
										...
										if ( childConnection === undefined ) return;
										const deformerParent = childConnection.parents.filter( function ( parent ) {

											return parent.relationship !== undefined;

										} )[ 0 ];
										if ( deformerParent === undefined ) return;
										const deformerID = deformerParent.ID;
										const deformerConnection = connections.get( deformerID );
										if ( deformerConnection === undefined || deformerConnection.parents.length === 0 ) return;
										const morpherID = deformerConnection.parents[ 0 ].ID;
										const morpherConnection = connections.get( morpherID );
										if ( morpherConnection === undefined || morpherConnection.parents.length === 0 ) return;
										const geoID = morpherConnection.parents[ 0 ].ID; // assuming geometry is not used in more than one model
										const geoConnection = connections.get( geoID );
										if ( geoConnection === undefined || geoConnection.parents.length === 0 ) return;

										const modelID = geoConnection.parents[ 0 ].ID;
