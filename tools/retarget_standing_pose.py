"""Bake the Female Standing Pose onto the original Sirious CC3 armature."""
...
CC3_FBX = PROJECT_ROOT / "assets/Blender/blender.Fbx"
SOURCE_FBX = PROJECT_ROOT / "assets/Female Standing Pose.fbx"
OUTPUT_DIR = PROJECT_ROOT / "assets/animations"
OUTPUT_GLB = OUTPUT_DIR / "female_standing_baked.glb"
DIAGNOSTIC_DIR = OUTPUT_DIR / "diagnostics"
DIAGNOSTIC_AUDIT = OUTPUT_DIR / "female_standing_retarget_audit.json"
...
def bake_world_space_retarget(
    ...
    source,
    target,
    action_name="CC3_Female_Standing_Baked",
...
def write_pose_audit(scene, source, target):
    ...
    unmapped_source = sorted(source_names - set(resolved_pairs))
    terminal_source = sorted(
        name
        for name in unmapped_source
        if name.endswith("4") or name.endswith("_End") or name.endswith("HeadTop_End")
    )
    unmapped_deforming = sorted(set(unmapped_source) - set(terminal_source))
    audit = {
        "source": str(SOURCE_FBX.relative_to(PROJECT_ROOT)),
        "target": str(CC3_FBX.relative_to(PROJECT_ROOT)),
        "source_action": source.animation_data.action.name,
        "source_frame_range": [scene.frame_start, scene.frame_end],
        "source_bones": len(source_names),
        "target_bones": len(target_names),
        "mapped_deforming_bones": len(resolved_pairs),
        "mapped_pairs": resolved_pairs,
        "unmapped_source_terminal_markers": terminal_source,
        "unmapped_source_deforming_bones": unmapped_deforming,
        "preserved_cc3_only_bones": sorted(target_names - set(resolved_pairs.values())),
...
def main():
    ...
    unmapped_source = source_names - resolved_source_names
    unmapped_deforming = sorted(
        name
        for name in unmapped_source
        if not (name.endswith("4") or name.endswith("_End") or name.endswith("HeadTop_End"))
    )
    if unmapped_deforming:
        raise RuntimeError(
            "Source deform bones have no exact CC3 mapping: "
            + ", ".join(unmapped_deforming)
        )
    ...
    render_diagnostic(
        DIAGNOSTIC_DIR / "cc3_standing_front.png",
        target_visuals,
        sample_frame,
        view_sign=-1,
    )
    render_diagnostic(
        DIAGNOSTIC_DIR / "cc3_standing_side.png",
        target_visuals,
        sample_frame,
        view_axis="X",
        view_sign=-1,
    ...
    render_diagnostic(
        DIAGNOSTIC_DIR / "cc3_standing_back.png",
        target_visuals,
        sample_frame,
        view_sign=1,
    )
    ...
    print(f"CC3_STANDING={OUTPUT_GLB}")
